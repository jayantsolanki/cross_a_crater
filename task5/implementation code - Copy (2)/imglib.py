...
def markobstacle(img,m,n):
    ...
    for x in range(0,m-1):
        ...
        for y in range(0,n-1):
            Y=y*n+(n/2)
            if img[X,Y]>=200 or x==0 or y==0 or y==n-2 or x==m-2:#and img[X,Y,1]>=70 and img[X,Y,2]>=70: #obstacle black ,bgr value(0,0,0)
                grid_map[x][y]=1
        ...
        for x in range(0,m-1):
                ...
                for y in range(0,n-1):
                        ...
                        if img[X,Y]>=70 or x==0 or y==0 or y==n-2 or x==m-2:#and img[X,Y,1]>=70 and img[X,Y,2]>=70: #obstacle black ,bgr value(0,0,0)
                                ...
                                continue
        #print grid_map
        grid_start = GridPoint((Bot_position[1][1]/m),(Bot_position[1][0]/n)) ##reversing coordinates so that it can be compatible with coordinate system of matrix

        grid_end_provision1 = GridPoint((provision[0][1]/m),(provision[0][0]/n))
        grid_end_provision2 = GridPoint((provision[1][1]/m),(provision[1][0]/n))
        grid_end_provision3 = GridPoint((provision[2][1]/m),(provision[2][0]/n))
        #grid_end_provision1=GridPoint(2,12)
        grid_end_bed_demand1 = GridPoint((bed_demand[0][1]/m)+1,(bed_demand[0][0]/n))
        grid_end_bed_demand2 = GridPoint((bed_demand[1][1]/m)+1,(bed_demand[1][0]/n))
        grid_end_bed_demand3 = GridPoint((bed_demand[2][1]/m)+1,(bed_demand[2][0]/n))
