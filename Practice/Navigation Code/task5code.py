...
length,route=solve(start,stop,frame)
execute(length,route)# going for yellow provision 2
ser.write("y")#for glowing yellow led
##

r1,r2=getcoor(marker[4][0],marker[4][1],n,m)# red table 3
print r1,r2
start=GridPoint(yy,yx)
stop=GridPoint(r2+1,r1)
length,route=solve(start,stop,frame)
execute(length,route)# going for red table
ser.write("O")#for glowing off red led
##
y1,y2=getcoor(marker[4][0],marker[4][1],n,m)# yellow table 4
start=GridPoint(r2+1,r1)
stop=GridPoint(y2+2,y1)
length,route=solve(start,stop,frame)
execute(length,route)# going for yellow table
ser.write("o")#for glowing off yellow led
##

start=GridPoint(y2+1,y1)
stop=GridPoint(by,bx)
length,route=solve(start,stop,frame)
execute(length,route)# going for blue provision 5
ser.write("b")#for glowing  blue led
##
b1,b2=getcoor(marker[5][0],marker[5][1],n,m)# blue table 6
